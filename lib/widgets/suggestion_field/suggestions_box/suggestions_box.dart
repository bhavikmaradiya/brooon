...
class SuggestionsBox {
  ...
  Future<bool> _waitChangeMetrics(bool isMounted) async {
    if (widgetMounted && isMounted) {
  ...
  void _adjustMaxHeightAndOrientation() {
    SuggestionsField widget = context.widget as SuggestionsField;
  ...
  Future<void> onChangeMetrics(bool isMounted) async {
    if (await _waitChangeMetrics(isMounted)) {
      resize();
    }
  }
}